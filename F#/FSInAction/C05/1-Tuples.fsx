...
let makeDoctor name =
  let _, sname = name
  "Dr. ", sname
let r1 = makeDoctor ("John", "Doe")

let makeDoctor' (name : string * string) : string * string =
  let _, sname = name
  "Dr. ", sname

// Deconstruction Inline
let makeDoctorDecon (_, sname) = 
  "Dr", sname
...
let parsed', theInt = System.Int32.TryParse("one")
if parsed' then printfn $"{theInt}" else printfn $"{theInt}"
