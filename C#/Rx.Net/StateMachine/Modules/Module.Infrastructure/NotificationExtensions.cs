...
public static class NotificationExtensions
{
  ...
  /// <summary>
  /// Returns an observable sequence of the value of a property when <paramref name="source"/>
  /// raises <seealso cref="INotifyPropertyChanged.PropertyChanged"/> for the given property.
  /// </summary>
  /// <typeparam name="T">The type of the source object. Type must implement <seealso cref="INotifyPropertyChanged"/>.</typeparam>
    ...
    Expression<Func<T, TProperty>> property) where T : INotifyPropertyChanged
  {
    ...
    {
      var propertyName_ = property.GetPropertyInfo().Name;
      var propertySelector_ = property.Compile();

      return Observable.FromEventPattern<PropertyChangedEventHandler, PropertyChangedEventArgs>(
          handler => handler.Invoke,
          h => source.PropertyChanged += h,
          h => source.PropertyChanged -= h)
        .Where(e => e.EventArgs.PropertyName == propertyName_)
        .Select(e => propertySelector_(source))
        .Subscribe(o);
    });
  }
  ...
  public static PropertyInfo GetPropertyInfo<TSource, TValue>(this Expression<Func<TSource, TValue>> property)
  {
    ...
    var body_ = property.Body as MemberExpression;
    if(body_ == null)
    {
      throw new ArgumentException("Expression is not a property", "property");
    }

    var propertyInfo_ = body_.Member as PropertyInfo;
